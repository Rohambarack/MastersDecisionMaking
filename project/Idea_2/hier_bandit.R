...
a_mean <- rgamma(1,1.5,15)#.1
a_sd <- rgamma(1,1,15) #0.01
beta_mean <- rgamma(1,1,15)#.05 # higher number means more consistent choice behavior (aka. less exploration)
beta_sd <- rgamma(1,2,15) #0.1
  


inter_mean <- rgamma(1,3,15) #0.2
inter_sd <- rgamma(1,1,15) #0.05

minor_mean <- rgamma(1,3,15) #0.3
minor_sd <- rgamma(1,2,15) #0.1
...
#                turn = c(1:72,1:72),
#                p_val = c(RW_sims$p[1,,1],
#                          RW_sims$p[1,,2]))
#agent assessment 
#test %>% 
#  ggplot(aes(y = p_val, x = turn, colour = arm))+
#  geom_point()+
